...
const Paiements = () => {
  ...
  const getMethodeBadge = (methode: string) => {
    ...
    switch (methode) {
      case 'mobile_money':
        return <Badge variant="outline" className={`${badgeClasses} border-blue-200 text-blue-700`}>Mobile Money</Badge>;
      case 'virement':
        return <Badge variant="outline" className={`${badgeClasses} border-green-200 text-green-700`}>Virement</Badge>;
      case 'especes':
        return <Badge variant="outline" className={`${badgeClasses} border-purple-200 text-purple-700`}>Espèces</Badge>;
      default:
                ...
                <TableBody>
                  {filteredPaiements.length > 0 ? filteredPaiements.map((paiement) => (
                    <TableRow key={paiement.id} className="hover:bg-muted/50">
                      <TableCell>
                        <div>
                          <div className="font-medium">{paiement.candidat_nom || 'Nom non disponible'}</div>
                          <div className="text-sm text-muted-foreground">{paiement.candidat_nip || 'NIP non disponible'}</div>
                        </div>
                      </TableCell>
                      <TableCell className="font-mono text-sm">{paiement.reference || `REF_${paiement.id}`}</TableCell>
