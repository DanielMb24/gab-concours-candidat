...
const Confirmation = () => {
  ...
  const handleContinuer = () => {
    if (numeroCandidature) {
      apiService.createSession(decodeURIComponent(numeroCandidature)).then(r => {});
      console.log('Redirecting to:', `/documents/${encodeURIComponent(decodeURIComponent(numeroCandidature))}`);
      navigate(`/documents/${encodeURIComponent(decodeURIComponent(numeroCandidature))}`);
    }
  };

  const copyToClipboard = () => {
    if (numeroCandidature) {
      navigator.clipboard.writeText(decodeURIComponent(numeroCandidature)).then(r => {});
      toast({
        title: "Copié !",
  ...
  const candidatureNumber = numeroCandidature ? decodeURIComponent(numeroCandidature) : 'Numéro non disponible';
